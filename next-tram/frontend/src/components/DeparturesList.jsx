    ...
    <div className="ntc2-card">
      ...
      <div className="ntc2-left">
        ...
        <div className="ntc2-identity">
          <span className="ntc2-badge">{departure.line || 'S1'}</span>
      ...
      <div className="ntc2-center">
        ...
        <div className="ntc2-num">
          {departure.cancelled ? 'X' : String(minutes).padStart(2, '0')}
        </div>
        <div className="ntc2-min-label">MIN</div>
      </div>

      {/* Right: departure time + status */}
      <div className="ntc2-right">
        <div className="ntc2-time-label">SCHEDULED</div>
        <div className="ntc2-deptime-big">{depTime}</div>
  ...
  return (
    <tr className={`tt2-row ${departure.cancelled ? 'tt2-row--cancelled' : ''}`}>
      <td className="tt2-line">
        <span className="tt2-badge">{departure.line || '?'}</span>
      </td>
      <td className="tt2-time">
        {departure.delay > 0 && departure.departure !== departure.departureActual ? (
          <><span className="tt2-time-planned">{departure.departure}</span> <span className="tt2-time-actual">{departure.departureActual}</span></>
        ) : (
          <span className="tt2-time-val">{departure.departureActual || departure.departure}</span>
        )}
      </td>
      <td className="tt2-arrow">→</td>
      <td className="tt2-in">
        {!departure.cancelled && mins !== null && (
          <span className="tt2-relative-wrap">
            <span className="tt2-rel-label">in </span>
            <span className="tt2-rel-num">{mins}</span>
            <span className="tt2-rel-label"> min</span>
          </span>
        )}
...
export default function DeparturesList({ departures }) {
  ...
  const nextIdx = sorted.findIndex(d => !d.cancelled)
  const next = nextIdx >= 0 ? sorted[nextIdx] : sorted[0]
  const rest = sorted.filter((_, i) => i !== (nextIdx >= 0 ? nextIdx : 0))
